...
const AttendeeAgeChart = () => {
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100 h-full flex flex-col">
      <div className="mb-3">
        <h3 className="text-lg font-bold text-gray-900 mb-3">ATTENDEE AGE</h3>
      ...
      <div className="flex-1 relative px-2 pb-6" style={{ minHeight: '250px' }}>
        {/* Chart columns */}
        <div className="h-full flex justify-between items-end relative">
          {data.map((item, index) => {
            // Calculate positions for each age group with better scaling
            const positions = item.values.map((value, i) => {
              if (value === 0) return null;
              return {
                value,
                color: ageGroups[i].color,
                // Better scaling to fit within container - reduce multiplier and offset
                bottom: 25 + (value * 2.8) + (i * 8) // Reduced scaling for better fit
              };
            }).filter(pos => pos !== null);
            ...
            return (
              <div key={index} className="relative flex flex-col items-center" style={{ width: '50px', height: '100%' }}>
                {/* Vertical connecting lines */}
                {positions.length > 1 && (
                  <div className="absolute w-1 bg-gray-300" 
                       style={{ 
                         left: '50%', 
                ...
                {positions.map((pos, posIndex) => (
                  <div
                    key={posIndex}
                    className="absolute w-7 h-7 rounded-full border-2 border-white shadow-sm flex items-center justify-center"
                    style={{
                  ...
                  >
                    <span className="text-white text-xs font-bold">{pos.value}</span>
                  </div>
                ))}
                ...
                {/* Date label */}
                <span className="absolute bottom-0 text-xs text-gray-500">{item.date}</span>
              </div>
            );
          })}
        </div>
